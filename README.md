...
- `commands.md` (L2) — the command set: `!reboot`, `!save`, `!load`, `!cast`, `!tree`, `!cleanup`, `!compress`, and more.
...
- `grimoire/` — persistent memory, organized into topic folders. A topic may nest subtopic folders (each just a `memory.md`), so you can load one facet of a topic without pulling the rest.
...
Each topic is a folder. `memory.md` (the topic's core) is always there; `tasks.md` (shared across the topic) and a `context/` folder of reference material appear only when there's content for them — nothing is pre-built. Big topics split into **subtopics**: nested folders that each hold only their own `memory.md`. `!load topic` pulls just the topic's `memory.md` and lists its subtopics; `!load topic/sub` pulls the shared root plus that subtopic; `!load topic/all` pulls everything. That way working on one facet (say, mechanics) never drags in the rest (say, lore).
